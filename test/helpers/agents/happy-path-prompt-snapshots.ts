...
const CODEX_WORKSPACE_BOOTSTRAP_CONTEXT_FILES: readonly PromptSnapshotFile[] = [];
...
const CODEX_WORKSPACE_BOOTSTRAP_PROMPT_CONTEXT =
  CODEX_WORKSPACE_BOOTSTRAP_CONTEXT_FILES.length > 0
    ? [
        "OpenClaw loaded these user-editable workspace files for the current turn. Codex loads AGENTS.md natively. SOUL.md, IDENTITY.md, TOOLS.md, USER.md, and MEMORY.md are provided as Codex thread developer instructions so standing workspace guidance is not repeated in every turn. HEARTBEAT.md is handled by heartbeat collaboration-mode guidance. Those files are not repeated here.",
        "",
        "# Project Context",
        "",
...
function renderScenarioSnapshot(scenario: PromptScenario): string {
  ...
  return [
    ...
    "- This also simulates Codex workspace bootstrap routing: `SOUL.md`, `IDENTITY.md`, `TOOLS.md`, `USER.md`, and `MEMORY.md` as Codex developer instructions, with `HEARTBEAT.md` as a heartbeat-only file pointer.",
    "",
    "## Scenario Metadata",
    "",
    markdownFence(
...
function renderReadme(scenarios: PromptScenario[]): string {
  return [
    ...
    "",
    "The workspace bootstrap simulation includes dummy workspace contents so prompt reviewers can see how OpenClaw routes profile and memory files into Codex developer instructions once per thread and points heartbeat turns at `HEARTBEAT.md` without inlining it. `AGENTS.md` is intentionally not repeated here because Codex loads it natively.",
    "",
