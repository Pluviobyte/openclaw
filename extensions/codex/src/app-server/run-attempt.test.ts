...
describe("runCodexAppServerAttempt", () => {
  ...
  it("preserves workspace instruction files when before_prompt_build replaces Codex developer instructions", async () => {
    ...
    const memorySummary = "Memory summary that must stay session-scoped.";
    ...
    expect(threadStartParams.developerInstructions).toContain(memorySummary);
  ...
  it("passes workspace identity and memory files as Codex thread developer instructions", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
